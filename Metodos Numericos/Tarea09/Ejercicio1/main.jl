...
p1=splineLineal(z,x,y1,m,n) #Calculamos la interpolación por Splines por los puntos (x,y) evaluada en z

plt1=plot(z,[f1,p1],label = [L"x + \frac{x\sin(\frac{x}{2})}{3}" "spline c=0"],legend=:topleft,ylims=(yMin,yMax),size=(800,600)); #Graficamos
savefig(plt1,@sprintf("pSplineLineal1-%i.png",n)) #Guardamos la gráfica
ecm1=norma2(p1-f1,m)/m; #Cálculo del Error Cuadrático Medio
println(@sprintf("El error cuadrático medio de la aproximación de f1 interpolando con splines lineales por %i puntos es de %g",n,ecm1))
