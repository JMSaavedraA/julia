...
p1=splineCuadraticoIzquierda(z,x,y1,m,n,1.0) #Calculamos la interpolación por Splines por los puntos (x,y) evaluada en z

plt1=plot(z,[f1,p1],label = [L"x + \frac{x\sin(\frac{x}{2})}{3}" "spline c=1"],legend=:topleft,ylims=(yMin,yMax),size=(800,600));
savefig(plt1,@sprintf("pSplineCuadratico1-%i.png",n)) #Guardamos la gráfica
ecm1=norma2(p1-f1,m)/m; #Cálculo del Error Cuadrático Medio
println(@sprintf("El error cuadrático medio de la aproximación de f1 interpolando con splines cuadráticos por %i puntos es de %g",n,ecm1))
