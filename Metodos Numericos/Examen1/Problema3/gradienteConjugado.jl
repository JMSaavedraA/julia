...
function gradienteConjugadoThreads(A::AbstractMatrix,b::AbstractVector,m::Int,tol::AbstractFloat,maxIter::Int)
    r=-copy(b)
    p=copy(b)
    w=zeros(Float64,m)
    x=zeros(Float64,m)
...
function gradienteConjugadoPrecondicionadoThreads(A::AbstractMatrix,b::AbstractVector,m::Int,tol::AbstractFloat,maxIter::Int)
    r=-copy(b)
    M=invierteDiagonalVector(A,m)
    y=zeros(Float64,m)
    for i=1:m
