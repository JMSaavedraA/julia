...
function multiplicaNdiagonalVectorThreads(A::AbstractMatrix,x::AbstractVector,c::AbstractVector,m::Int,n::Int)
    r=2*n+1
    for i=1:n
        c[i]=A[i,n+2-i:r]'*x[1:n+i]
    end
    Threads.@threads for i=n+1:m-n
        c[i]=A[i,:]'* x[i-n:i+n]
    end
    for i=n-1:-1:0
        c[m-i]=A[m-i,1:n+1+i]'*x[m-n-i:m]
...
function resuelveCholeskyPentadiagonal(Aaux::AbstractMatrix,baux::AbstractVector,m::Int,x::AbstractVector)
    L=CholeskyLDLPentadiagonal(Aaux,m)
    d=zeros(Float64,m)
    for i=1:m
        d[i]=L[i,3]
