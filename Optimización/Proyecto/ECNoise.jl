...
function ECNoise(nf::Integer,F::AbstractVector)
    #
#  Original ECNoise algorithm modified to work in Julia by Jose Miguel Saavedra Aguilar and Carlos Manuel Bosch Machado
...
#      inform = 3  Noise has not been detected; The interval is too large.
#      inform = 4  Noise has not been detected; h is too large.
...
#     Jose Miguel Saavedra Aguilar and Carlos Manuel Bosch Machado. May 2023
...
fmin = minimum(fval);    fmax = maximum(fval);
if (fmax-fmin)/max(abs(fmax),abs(fmin),1)>.1; 
    inform = 3;
...
for j = 1:nf-1
    ...
    level[j] = sqrt(γ*mean(fval[1:nf-j].^2));
    
    # Determine differences in sign.
    emin = minimum(fval[1:nf-j]);
    emax = maximum(fval[1:nf-j]);
    if (emin*emax < 0.0)
        dsgn[j] = true;
    end
end
...
for k = 1:nf-3
    emin = minimum(level[k:k+2]);
    emax = maximum(level[k:k+2]);
...
function ECNoise(nf::Integer, f::Function, h::AbstractFloat, x::Number)
    #
#  ECNoise algorithm modified to work in Julia by Jose Miguel Saavedra Aguilar and Carlos Manuel Bosch Machado. This algorithm has been modified to accept different inputs compared to the original algorithm above.
#  Determines the noise of a function from the function values
#
#     [fnoise,level,inform] = ECnoise(nf,f,h,x)
#
#  The user must provide the number of points to evaluate nf, the function handle f, the step length h and the point to evaluate x.
#
#  Noise will not be detected by this code if the function values differ
#  in the first digit.
#
...
#                  Try 100*h for the next value of h.
#      inform = 3  Noise has not been detected; The interval is too large.
#                  Try nf/2 for the next value of nf.
#      inform = 4  Noise has not been detected; h is too large.
#                  Try h/100 for the next value of h.
#
#     First Version
