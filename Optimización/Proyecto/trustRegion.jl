...
function trustRegionDogleg(f::Function,x0::AbstractVector)
  ...
  ϵ, _, _ = ECNoise(10,funNoise,h,0.0); # Variance estimate of the noise
  B = Matrix(I, n,n);   # Initial guess is the Identity Matrix, a SPD Matrix
  f0 = f(x);  # f(x_k)
  f1 = f0;    # f(x_{k+1})
  tol += sqrt(n)*ϵ/(2h) # Tolerance update to consider the noise of ∇f(x_k) approximation
  while (Ng>(tol)  && k < maxIter)
    d,c = dogleg(Δ,g,B); # Dogleg approximation of the quadratic model
    xs .= x + d;    # x_k + d_k
    f1 = f(xs);     # f(x_k + d_k)
    gs .= ∇(xs,f,h); # ∇f(x_k + d_k)
    ρ = ((f1-f0 - r*ϵ)/(.5*d'*B*d + g'*d - r*ϵ)); # The trust ratio ρ
